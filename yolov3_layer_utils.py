...
def upsample_layer(inputs, out_shape):
    ...
    #chaanels = tf.cast(tf.shape(inputs)[3], tf.int32)
    # NOTE: here height is the first
    inputs = tf.image.resize(inputs, (out_shape, out_shape))
    #inputs = tf.nn.conv2d_transpose(inputs, (3, 3, 256, 256), new_height, 2, padding='SAME')
    return inputs
