    ...
    def generator(self, image_id):
        ...
        if gt is None:
            ...
            for i in range(instance_num):
                vertical_ones = np.tile([[1]], [self.config.IMAGE_MAX_DIM//8, 1])
                # print(np.shape(vertical_ones))
                horizontal_ones = tf.tile([[1]], [1, self.config.IMAGE_MAX_DIM//8])
                pad_l = floor_x[i]
                pad_r = self.config.IMAGE_MAX_DIM//8- floor_x[i] - 1
                vertical_pads = np.pad(vertical_ones, [[0, 0], [pad_l, pad_r]], "constant", constant_values=(gt_left[i][0]//8, gt_right[i][0]//8))
                vertical_pads = np.expand_dims(vertical_pads, -1)
                # print(np.shape(vertical_pads))
                pad_t = floor_y[i]
                pad_b = self.config.IMAGE_MAX_DIM//8 - floor_y[i] -1
                horizontal_pads = np.pad(horizontal_ones, [[pad_t, pad_b], [0, 0]], "constant", constant_values=(gt_top[i][0]//8, gt_bot[i][0]//8))
